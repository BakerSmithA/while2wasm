...
fold :: (Functor f, Functor g, Monad m) => Alg f g m a -> ProgT f g m (m (a n)) -> m (a n)
fold alg prog = do
    x <- runProgT prog
    case x of
        Var   x -> x
...
run :: (Monad m, Functor f, Functor g) => (r -> m (a 'Z)) -> Alg f g m a -> ProgT f g m r -> m (a 'Z)
