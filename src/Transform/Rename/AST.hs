...
type Op  f = Rename    Ident :+: f
type Sc  g = LocalName Ident :+: g
-- type Ctx = Prog Op Sc
...
data Carrier f g a n
    = Rn { runRn :: Prog (Op f) (Sc g) (Carrier' f g a n) }
...
instance (Functor f, Functor g, VarExp Ident :<: f) => OpAlg (VarExp Ident) (Carrier f g a) where
    alg (GetVar v) = Rn $ do
        -- v' <- name v
        getVar v
